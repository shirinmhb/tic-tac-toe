...
def play_with_agent(agentWeights):
  agent = Agent('X', 'O', agentWeights, False)
  stateBoard = [[' ',' ',' '],[' ',' ',' '],[' ',' ',' ']]
  gameStates = []
  while(True):
    print('agent Turn:\n')	
    stateBoard = agent.find_best_move(stateBoard)
    gameStates.append(stateBoard)
    print_board(stateBoard)
    finished, winner = agent.is_game_finished(stateBoard)
    if finished:
      if winner == 'X':
        msg = 'agent won the game'
      elif winner == 'tie':
        msg = 'no one won the game'
      else:
        msg = 'you won the game, congrates!'
      print(msg)
      return(gameStates, msg, agent.weights)

    print('your Turn:\n')	
    print('Choose row(0-2):')
    x = int(input())
    print('Choose column(0-2):')
    y = int(input())
    stateBoard[y][x] = 'O'
    gameStates.append(stateBoard)
    print_board(stateBoard)
...
def play_with_agent2(agentWeights):
  agent = Agent('O', 'X', agentWeights, False)
  stateBoard = [[' ',' ',' '],[' ',' ',' '],[' ',' ',' ']]
  gameStates = []
  while(True):
    print('your Turn:\n')	
...
trained_weight = trian_agents()
while True:
  play_with_agent(trained_weight)
